...
export default function Settings({ 
  ...
  const handleAnalyzePortfolio = async () => {
    if (!portfolioUrl) {
      setPortfolioError("Please enter a valid portfolio URL.");
      return;
    }
    ...
    try {
      const aestheticDNA = await analyzePortfolioAesthetic(portfolioUrl, settings);
      setSettings(prev => ({
        ...prev,
        creatorProfile: {
          portfolioUrl,
      ...
      setPortfolioError(error.message || "Failed to analyze portfolio.");
    } finally {
      ...
      <div className="space-y-8">
        ...
        >
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h2 className="text-xl font-bold text-white font-display">Creator Profile</h2>
              <p className="text-sm text-slate-500 font-light">Extract your Aesthetic DNA for precise targeting.</p>
            </div>
          </div>

          <div className="space-y-6">
            <div>
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 block">Portfolio URL (e.g., Adobe Stock, Behance)</label>
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 bg-black/40 border border-white/5 rounded-2xl text-white px-6 py-4 outline-none focus:border-accent transition-all font-mono text-sm"
                />
                <button
                  onClick={handleAnalyzePortfolio}
                  disabled={isAnalyzingPortfolio || !portfolioUrl}
                  className="px-6 py-4 bg-accent/10 text-accent font-bold rounded-2xl border border-accent/20 hover:bg-accent hover:text-slate-900 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
              ...
              {portfolioError && (
                <p className="text-red-400 text-xs mt-2 flex items-center gap-1">
                  <AlertCircle size={12} /> {portfolioError}
                </p>
              )}
            </div>

            {settings.creatorProfile?.aestheticDNA && (
              <div className="p-5 bg-slate-900/40 rounded-2xl border border-white/5 relative">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-bold text-accent uppercase tracking-widest">Extracted Aesthetic DNA</h3>
                  <div className="flex items-center gap-4">
                    {settings.creatorProfile.lastAnalyzed && (
                      <span className="text-[10px] text-slate-500">
                        Updated: {new Date(settings.creatorProfile.lastAnalyzed).toLocaleDateString()}
                      </span>
                    )}
                    <button 
                      onClick={() => {
                        setSettings(s => ({ ...s, creatorProfile: undefined }));
                        setPortfolioUrl('');
                      }}
                    ...
                    >
                      <Trash2 size={14} />
                      <span className="hidden sm:inline">Clear Profile</span>
                    </button>
                  </div>
                </div>
